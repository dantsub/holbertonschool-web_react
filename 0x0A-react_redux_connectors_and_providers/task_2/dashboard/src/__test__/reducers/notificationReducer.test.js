...
describe('Test notificationReducer.js', () => {
  ...
  it('Test that FETCH_NOTIFICATIONS_SUCCESS returns the data passed with isRead in false', () => {
    ...
    expect(result.toJS().notifications).toEqual(notificationsNormalizer(defaultNotifications).notifications);
    expect(result.toJS().filter).toEqual(initialState.filter);
  });
  ...
  it('Test that MARK_AS_READ returns the data passed with index passed in true', () => {
    ...
    expect(result.notifications).toEqual(expected.notifications);
